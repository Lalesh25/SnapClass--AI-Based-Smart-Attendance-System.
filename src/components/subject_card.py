...
def subject_card(name, code, section, stats=None, footer_callback=None):
    ...
    if stats:
        ...
        for icon, label, value in stats:
            html+= f'<div style="background: #EB459E10; padding:5px 12px; border-radius:12px; font-size:0.9rem; color:#000;">{icon} <b style="color:#000;">{value}</b> <span style="color:#000;">{label}</span> </div>'
